...
namespace davidxu\alipay;

use davidxu\alipay\lotus\LtInflector;
use davidxu\alipay\lotus\LtLogger;
use \Exception;
...
class Alipay
{
     ...
     * 存放日志，AOP缓存数据
     */
    const AOP_SDK_WORK_DIR  = '/tmp/';
    const AOP_SDK_DEV_MODE  = true;
    ...
    protected $alipaySdkVersion = "davidxu-alipay-sdk-php-20170413";
     ...
     * @param boolean $keyfromfile 私钥获取方式，读取字符串还是读文件
     * @return string
     * @author mengyu.wh
     */
    ...
    protected function logCommunicationError($apiName, $requestUrl, $errorCode, $responseTxt) {
        ...
        $logger = new LtLogger();
        $logger->conf["log_file"] = rtrim(self::AOP_SDK_WORK_DIR, '\\/')
            . '/' . "logs/aop_comm_err_"
            . $this->appId . "_"
            . date("Y-m-d")
            . ".log";
...
//		$this->echoDebug("<br/> responseContent:<xmp>" . $responseContent . "</xmp>");
     ...
     * @param object $request
     * @param $responseContent
     * @return object EncryptParseItem|null
     */
     ...
     * @param $nodeName
     * @param $nodeIndex
     * @return object EncryptParseItem
     */
...
//		$this->echoDebug("<br/> responseContent:<xmp>" . $responseContent . "</xmp>");
     ...
     * @return string
     */
    private function encrypt($str, $secretKey) {
        //AES, 128 模式加密数据 CBC
        $secretKey = base64_decode($secretKey);
        $str = trim($str);
//        $str = $this->addPKCS7Padding($str);
//        $iv = @mcrypt_create_iv(@mcrypt_get_iv_size(MCRYPT_RIJNDAEL_128,MCRYPT_MODE_CBC),1);
//        $encrypt_str =  mcrypt_encrypt(MCRYPT_RIJNDAEL_128, $secretKey, $str, MCRYPT_MODE_CBC);
        $encryptStr = openssl_encrypt($str, 'aes-256-cbc', $secretKey, 0);
     ...
     * @return string
     */
    private function decrypt($str, $secretKey){
        //AES, 128 模式加密数据 CBC
        $str = base64_decode($str);
        $secretKey = base64_decode($secretKey);
//        $iv = mcrypt_create_iv(mcrypt_get_iv_size(MCRYPT_RIJNDAEL_128,MCRYPT_MODE_CBC),1);
//        $encrypt_str =  mcrypt_decrypt(MCRYPT_RIJNDAEL_128, $screct_key, $str, MCRYPT_MODE_CBC);
        $decryptStr = openssl_decrypt($str, 'aes-256-cbc', $secretKey, 0);
        $decryptStr = trim($decryptStr);
//        $decryptStr = $this->stripPKSC7Padding($decryptStr);
        return $decryptStr;
